  ...
  return {
    tools: [
      ...
      // === AI-powered tools (Gemini API or Ollama) ===
      {
        name: "recommend_dish",
        description:
          "Uses Gemini AI to recommend a dish from FoodExpress restaurants based on the user's preferences, mood, dietary needs, or cravings.",
        inputSchema: {
          type: "object",
          properties: {
            preferences: {
              type: "string",
              description:
                "What the user wants (e.g. 'something spicy and vegetarian under 300 rupees' or 'comfort food for a rainy evening').",
            },
          },
      ...
      },
      {
        ...
        description:
          "Uses Gemini AI to summarize the current user's order history into a friendly paragraph with insights about their eating habits.",
        inputSchema: { type: "object", properties: {} },
      },
      {
        name: "smart_order",
        description:
          "Uses Gemini AI to decide what to order based on a natural-language request, then actually places the order with FoodExpress. Example request: 'Order me something cheap and vegetarian, deliver to my home address.'",
        inputSchema: {
          type: "object",
          properties: {
            request: {
              type: "string",
              description:
                "Natural language description of what to order and where to deliver it.",
            },
            delivery_address: {
              type: "string",
  ...
  try {
    switch (name) {
      case "login": {
        ...
        if (authToken) updateAuthHeader(authToken);
...
Speak directly to the user in a casual tone.`;
      ...
      case "smart_order": {
        ...
        const { request: userRequest, delivery_address } = args as any;
